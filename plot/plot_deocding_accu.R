...
pathResults <- '/Users/yiyang/DATA/numMVPA_bids/outputs/derivatives/CoSMoMVPA'
mvpa <- read.csv(paste(pathResults, 'numMVPADecoding_s0_ratio500_202303261821.csv', sep ='/'))

pathResults <- '/Users/yiyang/DATA/numMVPA_analysis/outputs/derivatives/CoSMoMVPA'
mvpa_26 <- read.csv(paste(pathResults, 'numMVPADecoding_s0_ratio150_202303271511.csv', sep ='/'))
...
mvpa$roi_order <- ifelse(mvpa$roi == 'lIPS1000voxel', 1,
                         ifelse(mvpa$roi == 'rIPS1000voxel', 2, 99))
...
subsetmvpa = subset(mvpa, image == img)
...
########### now for data with 2.6mm voxel size
mvpa_26 <- mvpa_26[-c(8:9)]
mvpa_26$subID <-as.factor(mvpa_26$subID)
...
#name change from mask to roi
mvpa_26$mask <-as.factor(mvpa_26$mask)
names(mvpa_26)[2] <- 'roi'

# let's make a expType to split the no_pitch exp from pitch exp
mvpa_26$subNb <- as.numeric(mvpa_26$subID)

mvpa_26$roi_order <- ifelse(mvpa_26$roi == 'neurosynth_lIPS', 1,
                         ifelse(mvpa_26$roi == 'neurosynth_rIPS', 2, 99))

# think about other ways of ordering with below function
# mvpa$roi_order <- grepl('nopitch', mvpa$roi, fixed = TRUE)

# currently we  don't have 2 hemispheres for every ROI
mvpa_26$hemis <- ifelse(substr(mvpa_26$roi,12,12) == 'l', 'left',
                     ifelse(substr(mvpa_26$roi,12,12) == 'r', 'right',NA))

# make everything factor
str(mvpa_26)
mvpa_26$image<- as.factor(mvpa_26$image)
mvpa_26$hemis<-as.factor(mvpa_26$hemis)
mvpa_26$decodingCondition<-as.factor(mvpa_26$decodingCondition)
# subset the dataframe for plotting/analysis
img = 'beta' # or 't_maps'

subsetmvpa_26 = subset(mvpa_26, image == img)
str(subsetmvpa_26)

total_mvpa = rbind(subsetmvpa, subsetmvpa_26)
str(total_mvpa)
total_mvpa$voxelsz <- ifelse(total_mvpa$choosenVoxNb == '500', '1.5',
                        ifelse(total_mvpa$choosenVoxNb == '150', '2.6',99))

total_mvpa$voxelsz<- as.factor(total_mvpa$voxelsz)

df <- summarySE(data = subsetmvpa_26, 
                groupvars=c('decodingCondition','roi'),
                measurevar='accuracy', na.rm = TRUE)
df
######### analyze the data ######### 
results = leveneTest(accuracy ~ roi, subsetmvpa_26)
results
...
lIPS_26 <-subset(subsetmvpa_26, roi =='neurosynth_lIPS')
rIPS_26 <-subset(subsetmvpa_26, roi =='neurosynth_rIPS')
...
# sig dif than zero?
t.test(lIPS_26$accuracy, mu = 0.25, alternative = 'greater') # t = 5.498, df = 14, p-value = 3.925e-05
t.test(rIPS_26$accuracy, mu = 0.25, alternative = 'greater') # t = 2.0319, df = 14, p-value = 0.0308
...
fig1 <- ggplot(NULL, aes(x = decodingCondition:hemis, y = accuracy)) +
  geom_bar(data = subsetmvpa, aes(fill = hemis),
           alpha = 0.5, stat='summary', fun = 'mean', fill = 'grey',
           width = 0.7) +
  geom_bar(data = subsetmvpa_26, aes(fill = hemis),
