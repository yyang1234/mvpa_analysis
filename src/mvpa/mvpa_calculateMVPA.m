function accu = mvpa_calculateMVPA(opt)
...
for iSub = 1:numel(opt.subjects)
    ...
    for iImage = 1:length(opt.mvpa.map4D)
        ...
        for iMask = 1:length(subMasks)
            ...
            % choose the mask
            mask = fullfile(opt.dir.rois, subFolder, opt.mask, subMasks{iMask});
            ...
            for pair = 1:size(decodingPairs,1)
                ...
                fprintf(['Sub'  subID ': ' condLabelName{decodingPairs(pair,1)} ' v. ' condLabelName{decodingPairs(pair,2)} ...
                      ' v. ' condLabelName{decodingPairs(pair,3)} ' v. ' condLabelName{decodingPairs(pair,4)} ...
